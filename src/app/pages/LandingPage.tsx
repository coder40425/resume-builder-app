...
import { FileText, Download, Eye, Zap, Shield, Layout, ArrowRight, CheckCircle, Menu, X, Mail, Phone, MapPin, Linkedin, Github } from "lucide-react";
              ...
              {experience.map((exp: any) => (
                <div key={exp.id}>
                  ...
                  <ul className="list-disc list-outside ml-4 space-y-0.5 text-gray-700 text-xs">
                    {exp.description.map((desc: string, idx: number) => <li key={idx}>{desc}</li>)}
                  </ul>
                </div>
              ))}
            </div>
